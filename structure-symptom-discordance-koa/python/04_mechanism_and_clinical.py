...
DEFAULT_DERIVED = Path(os.environ.get("TABLES_DIR", str(PROJECT_ROOT / "outputs" / "tables")))
DEFAULT_OPT = Path(os.environ.get("OPTIMIZATION_DIR", str(PROJECT_ROOT / "outputs" / "optimization")))
DEFAULT_OAI_ZIP = Path(os.environ.get("OAI_ZIP", str(PROJECT_ROOT / "data" / "OAICompleteData_ASCII.zip")))
DEFAULT_OUT = Path(os.environ.get("MECHANISM_OUTPUT_DIR", str(PROJECT_ROOT / "outputs" / "mechanisms")))
...
def product_of_coefficients(df, exposure_col, mediator_col, y_col, covars, weight_col):
...
def task1_moaks(d: pd.DataFrame, z: zipfile.ZipFile, out: Path, B: int):
    ...
    for mediator in ["effusion_synovitis", "hoffa_synovitis_proxy", "bml_size_sum"]:
        for exposure in ["syd_vs_cl", "structure_score"]:
            res = product_of_coefficients(mri, exposure, mediator, "pain0", med_covars, "mri_selection_ipw")
    ...
    for b in range(B):
        ...
        for mediator in ["effusion_synovitis", "hoffa_synovitis_proxy", "bml_size_sum"]:
            res = product_of_coefficients(boot, "syd_vs_cl", mediator, "pain0", med_covars, "mri_selection_ipw")
    ...
    write_csv(med, out / "task1_ipw_association_decomposition.csv")
    write_csv(boot, out / f"task1_decomposition_bootstrap_B{B}.csv")
    ...
    fig.tight_layout()
    fig.savefig(out / "task1_moaks_decomposition_figure.png")
    plt.close(fig)
    return med
...
def main():
    ...
    with zipfile.ZipFile(args.oai_zip) as z:
        ...
        manifest["tasks"].append({"task": "1 baseline MOAKS IPSW association decomposition", "status": "completed", "outputs": "task1_moaks_*; task1_ipw_association_decomposition.csv"})
        ...
        manifest["tasks"].append({"task": "4 non-knee pain proxy analysis", "status": "completed proxy analysis", "outputs": "task4_*"})
    (args.out / "analysis_manifest.json").write_text(json.dumps(manifest, indent=2, ensure_ascii=False))
