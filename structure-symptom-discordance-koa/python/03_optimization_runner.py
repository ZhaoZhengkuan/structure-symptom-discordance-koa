...
Tier B: B1 landmark, B3 TKR truncation audit, B5 OAI cohort adjustment/audit.
Tier D: D2 imaging stewardship first-pass decision model, D3 health-equity subgroup.
Tier E: E2 continuous discordance index.
Tier F: reporting checklist skeletons, environment artifacts are generated by shell.

Skipped by user instruction: E1 MOST external validation.
Deferred because extra phenotype-specific source extraction is needed:
C1 MOAKS mediation, C2 FNIH biomarkers, C3 central sensitization signatures, D1 post-TKR pain.
"""
...
def b1_landmark(panel, baseline, outcomes):
    ...
    ph["landmark_phenotype"] = ph.apply(lab, axis=1)
    d = baseline.merge(ph[["kid", "landmark_phenotype"]], on="kid").merge(outcomes, on=["kid", "id", "side"])
    lm_days = 24 * 30.4375
    d = d[~((d["tkr_event"] == 1) & d["tkr_days"].notna() & (d["tkr_days"] <= lm_days))].copy()
    d["event_after_landmark_96"] = ((d["tkr_event"] == 1) & d["tkr_days"].notna() & (d["tkr_days"] > lm_days) & (d["tkr_days"] <= HORIZON_DAYS)).astype(int)
    rows = []
    for pheno in PHENO_ORDER:
        g = d[d["landmark_phenotype"] == pheno]
        rows.append({"landmark_phenotype": pheno, "n": len(g), "events_after_24m_to_96m": int(g["event_after_landmark_96"].sum()), "risk_percent": 100 * g["event_after_landmark_96"].mean() if len(g) else np.nan})
    pd.DataFrame(rows).to_csv(OUT / "B1_landmark_24m_tkr_risk.csv", index=False)
    pd.DataFrame([si, yi]).to_csv(OUT / "B1_landmark_24m_model_selection.csv", index=False)
    ph.to_csv(OUT / "B1_landmark_24m_phenotypes.csv", index=False)
...
def deferred_status():
    rows = [
        {"task": "C1 MOAKS MRI mediation", "status": "deferred_data_extraction", "reason": "requires MOAKS variable harmonisation/IPW MRI subset"},
        {"task": "C2 FNIH biomarkers", "status": "deferred_data_extraction", "reason": "requires biomarker subcohort linkage and marker selection"},
        {"task": "C3 central sensitization/pain signatures", "status": "deferred_data_extraction", "reason": "requires additional pain-frequency/widespread-pain variables"},
        {"task": "D1 post-TKR residual pain", "status": "deferred_outcome_definition", "reason": "requires post-TKR visit alignment and post-operative WOMAC/satisfaction definition"},
        {"task": "E1 MOST external validation", "status": "skipped_by_user_instruction", "reason": "external MOST data not available"},
        {"task": "B2 shared-random-effects joint model", "status": "partially_addressed", "reason": "lcmm joint latent trajectories + Fine-Gray completed; full JMbayes2 shared-random-effects survival model remains optional server run"},
    ]
    pd.DataFrame(rows).to_csv(OUT / "deferred_and_skipped_tasks.csv", index=False)
...
def main():
    ...
    a3_ipcw(d)
    b1_landmark(panel, baseline, outcomes)
    ...
    manifest = {
        ...
        "partially_or_elsewhere_completed": ["A4 via R MICE/Fine-Gray cluster bootstrap outputs", "A6 bootstrap scripts/results", "A7 Fine-Gray + cause-specific Cox pending R script", "B5 status generated"],
