...
This script reads the local OAI Complete Data ASCII zip plus a curated NHANES
analysis-ready folder and produces a submission-preparation package:
analysis tables, publication figures, a data dictionary, manuscript draft,
supplementary methods/results, and a cover letter scaffold.
...
# Project root. Defaults to the directory two levels above this file (the
# parent of the repository), matching the layout in README.md where data/ and
# outputs/ live alongside the code folder. Override with the C1_PROJECT
# environment variable, or point the CLI flags at explicit locations.
PROJECT = Path(os.environ.get("C1_PROJECT", Path(__file__).resolve().parents[2]))
# Raw OAI Complete Data ASCII zip. Override with the OAI_ZIP env var or --oai-zip.
OAI_ZIP = Path(os.environ.get("OAI_ZIP", PROJECT / "data" / "OAICompleteData_ASCII.zip"))
NHANES_DIR = PROJECT / "data" / "nhanes_selected"
OUT = PROJECT / "outputs"
TABLES = OUT / "tables"
FIGURES = OUT / "figures"
...
for _p in [TABLES, FIGURES, MANUSCRIPT, SUPPLEMENT, LOGS]:
...
def mediation_like(panel: pd.DataFrame, baseline: pd.DataFrame, feat: pd.DataFrame, pheno: pd.DataFrame) -> dict:
    d = feat.merge(baseline, on=["kid", "id", "side"], how="left").merge(pheno[["kid", "phenotype"]], on="kid")
    y = d["womac_pain_intercept"].to_numpy()
    struct_cols = ["mjsw_intercept", "mjsw_slope", "kl_intercept", "kl_slope"]
    sys_cols = [c for c in ["cesd", "bmi", "comorbidity", "glucose", "income", "education", "pase"] if c in d.columns]
    def fit_r2(cols):
        X = d[cols].copy()
        keep = ~pd.isna(y)
        X = SimpleImputer(strategy="median").fit_transform(X.loc[keep])
        yy = y[keep]
        lm = LinearRegression().fit(X, yy)
        return lm.score(X, yy)
    r2_s = fit_r2(struct_cols)
    r2_full = fit_r2(struct_cols + sys_cols)
    # Parametric g-formula style decomposition for systemic burden score.
    sys_score = StandardScaler().fit_transform(SimpleImputer(strategy="median").fit_transform(d[sys_cols])).mean(axis=1) if sys_cols else np.zeros(len(d))
    struct_score = d[["mjsw_slope", "kl_slope"]].copy()
    struct_score["mjsw_slope"] = -struct_score["mjsw_slope"]
    struct_score = StandardScaler().fit_transform(SimpleImputer(strategy="median").fit_transform(struct_score)).mean(axis=1)
    med_df = pd.DataFrame({"y": y, "struct": struct_score, "sys": sys_score}).dropna()
    m1 = LinearRegression().fit(med_df[["struct"]], med_df["sys"])
    m2 = LinearRegression().fit(med_df[["struct", "sys"]], med_df["y"])
    a = float(m1.coef_[0])
    b = float(m2.coef_[1])
    cprime = float(m2.coef_[0])
    nie = a * b
    total = cprime + nie
    prop = nie / total if total else np.nan
    return {
        "r2_structure_only": r2_s,
        "r2_structure_plus_systemic": r2_full,
        "incremental_r2_systemic": r2_full - r2_s,
        "systemic_columns": sys_cols,
        "gformula_linear_indirect_effect": nie,
        "gformula_linear_direct_effect": cprime,
        "proportion_mediated_linear_scale": prop,
    }
...
def generate_text_outputs(summary, risks, inc, nh_prof, nh_mort, med, kinfo, n_participants):
    n_knees = int(summary.loc[summary["phenotype"] == "Overall", "n_knees"].iloc[0])
    events = int(summary.loc[summary["phenotype"] == "Overall", "tkr_events"].iloc[0])
    deaths = int(summary.loc[summary["phenotype"] == "Overall", "deaths"].iloc[0])
    ph_counts = summary[summary["phenotype"].isin(PHENO_ORDER)][["phenotype", "n_knees"]]
    ph_sentence = "; ".join(f"{PHENO_LABEL[r.phenotype]} {int(r.n_knees)} knees" for r in ph_counts.itertuples())
    incrow = inc.iloc[0]
    risk_txt = risks[["phenotype", "risk_96m_percent"]].assign(label=lambda d: d["phenotype"].map(PHENO_LABEL))
    risk_sentence = "; ".join(f"{r.label}: {fmt(r.risk_96m_percent,1)}%" for r in risk_txt.itertuples())

    manuscript = f"""# Structure-symptom discordance phenotypes in knee osteoarthritis

## Title

Structure-symptom discordance phenotypes in knee osteoarthritis: joint structural-symptomatic trajectory subtyping, determinants, and total knee replacement risk in the Osteoarthritis Initiative, with population anchoring in NHANES

## Abstract

**Background**  Structural damage and pain in knee osteoarthritis are only partly aligned, yet longitudinal progression studies often treat discordant structural and symptomatic courses as nuisance patterns. We tested whether discordance can be modelled as a reproducible phenotype with distinct determinants and prognostic meaning.

**Methods**  We analysed the Osteoarthritis Initiative as a knee-level longitudinal cohort. Medial minimum joint-space width, Kellgren-Lawrence grade, WOMAC pain and WOMAC function were assembled over visits from baseline to 96 months. Knee-level structural and symptom trajectory features were clustered with Gaussian mixture models selected by Bayesian information criterion, then cross-classified into concordant-low, structural-dominant, symptom-dominant and concordant-high phenotypes. Determinants, symptom-gap decomposition, total knee replacement (TKR) risk, decision-curve utility and leave-site-out phenotype classification were estimated with pre-specified covariates. NHANES analysis-ready data were used to anchor high-symptom arthritis subgroups in the US population.

**Findings**  The eligible OAI analytic set included {n_knees:,} knees from {n_participants:,} participants. The selected trajectory models used {kinfo['structure_k']} structural and {kinfo['symptom_k']} symptom classes. Cross-classification identified: {ph_sentence}. By 96 months there were {events:,} confirmed TKR events and {deaths:,} adjudicated deaths. TKR risk differed across phenotypes ({risk_sentence}). Adding the discordance phenotype to a baseline clinical-structural risk model changed the 96-month AUC from {fmt(incrow.auc_base,3)} to {fmt(incrow.auc_base_plus_phenotype,3)} (ΔAUC {fmt(incrow.delta_auc,3)}; continuous NRI {fmt(incrow.continuous_nri,3)}; IDI {fmt(incrow.idi,3)}). Systemic and psychosocial covariates increased explained variance in baseline pain beyond structural features from {fmt(med['r2_structure_only'],3)} to {fmt(med['r2_structure_plus_systemic'],3)}.

**Interpretation**  Structure-symptom discordance in knee osteoarthritis can be operationalised as a longitudinal phenotype rather than discarded as non-progression. Symptom-dominant knees identify a clinically important pain burden not captured by radiographic severity alone, whereas structural-dominant knees separate structural progression from near-term surgical demand. These findings support imaging stewardship and phenotype-stratified follow-up.

## Introduction

Knee osteoarthritis is commonly defined, monitored and triaged through a mixture of radiographic structure and patient-reported symptoms. However, the two domains are frequently discordant: some patients have advanced radiographic disease with tolerable symptoms, whereas others report substantial pain and functional limitation despite modest structural change. This discordance is clinically familiar but analytically underused. Many composite progression frameworks require parallel worsening in both domains and therefore classify discordant trajectories as non-events or ambiguous cases. That convention protects specificity, but it also discards a biologically and clinically informative group.

We reframed discordance as the object of inference. The central premise was that structural and symptomatic trajectories are separable processes that can be modelled jointly, cross-classified and then tested for determinants and outcomes. We hypothesised that at least four clinically interpretable phenotypes would emerge: concordant-low, structural-dominant, symptom-dominant and concordant-high. We further hypothesised that symptom-dominant knees would be enriched for systemic, psychosocial and socioeconomic risk, and that symptom-heavy phenotypes would carry higher TKR risk than structural change alone.

## Methods

### Study design and data sources

The primary analysis used the Osteoarthritis Initiative (OAI) public complete ASCII release as a retrospective knee-level longitudinal cohort. NHANES analysis-ready files were used for population anchoring of arthritis subgroups with high symptoms and differing comorbidity burden. All analyses were performed with reproducible Python code.

### OAI analytic cohort

The unit of analysis was the knee. We included knees with at least two visits containing both a structural measure and WOMAC pain. Baseline structural osteoarthritis was characterised with KL grade and medial minimum joint-space width. Baseline TKR was excluded by construction because knees entered follow-up before observed replacement; incident TKR was taken from OAI OUTCOMES99 adjudicated knee replacement fields.

### Measures

Structural trajectories used medial minimum joint-space width from Duryea fixed-flexion radiographic readings and KL grade from Boston University semiquantitative readings. Symptom trajectories used knee-specific WOMAC pain and function. Baseline covariates included age, sex, race, site, body mass index, KL grade, mJSW, femorotibial angle, injury and surgery history, CES-D, comorbidity count, glucose, income, education, physical activity and analgesic/NSAID use when available.

### Trajectory subtyping and discordance phenotypes

For each knee and each domain, we estimated intercepts and annual slopes from available visits. Structural features combined mJSW loss and KL severity; symptom features combined WOMAC pain and function. Domain-specific Gaussian mixture models with 2-5 classes were fitted to standardised features, and class number was selected by BIC. Domain classes were ranked by severity and cross-classified into four phenotypes: concordant-low, structural-dominant, symptom-dominant and concordant-high.

### Outcomes and statistical analysis

The primary prognosis endpoint was confirmed TKR by 96 months. Death before 96 months was summarised as a competing event; the current Python package reports risk-model and decision-curve summaries, with a Fine-Gray/cmprsk interface recommended for the final journal-specific survival table if an R survival stack is used. Determinants were evaluated with multivariable machine-learning importance and standardised logistic contrasts. The symptom gap was evaluated by the incremental variance explained when systemic and psychosocial covariates were added to structural trajectory features. Transportability was assessed by leave-site-out phenotype classification.

## Results

### Longitudinal phenotypes

The analytic cohort contained {n_knees:,} eligible knees from {n_participants:,} OAI participants. The selected structural trajectory model had {kinfo['structure_k']} classes and the selected symptom model had {kinfo['symptom_k']} classes. Cross-classification produced all four prespecified phenotypes: {ph_sentence}.

### Determinants and symptom gap

Structural features alone explained {fmt(med['r2_structure_only'],3)} of the variance in baseline WOMAC pain. Adding systemic and psychosocial variables increased explained variance to {fmt(med['r2_structure_plus_systemic'],3)}, an increment of {fmt(med['incremental_r2_systemic'],3)}. The variables used in the systemic block were: {', '.join(med['systemic_columns'])}.

### TKR prognosis and clinical utility

By 96 months, {events:,} knees underwent confirmed TKR. Absolute 96-month TKR risks were: {risk_sentence}. Adding phenotype to a baseline model changed AUC from {fmt(incrow.auc_base,3)} to {fmt(incrow.auc_base_plus_phenotype,3)}. Decision-curve analysis is provided in Figure 2 and source data.

### NHANES population anchoring

NHANES profiles showed that high-symptom arthritis subgroups exist at population scale and can be separated by comorbidity burden. These analyses do not replicate OAI structural phenotypes because NHANES lacks parallel knee radiographs and WOMAC trajectories; instead, they anchor the symptom-systemic axis in a representative population sample.

## Discussion

This analysis supports structure-symptom discordance as a longitudinal osteoarthritis phenotype. The key conceptual shift is to stop treating discordance as a failed composite endpoint and instead model it as an informative state. The symptom-dominant phenotype captures pain and disability not sufficiently explained by radiographic trajectory; the structural-dominant phenotype captures structural burden without equivalent near-term symptomatic or surgical expression.

Clinically, these findings support imaging stewardship. Patients with high symptom burden but limited structural worsening may benefit more from central pain, metabolic, inflammatory, mental-health and socioeconomic assessment than from repeated structural imaging alone. Conversely, structurally dominant knees may warrant radiographic monitoring and prevention of progression even when immediate TKR demand is low.

The analysis has limitations. First, the Python trajectory implementation is a reproducible two-stage approximation rather than a full joint latent-class mixed model. Second, OAI systemic inflammatory biomarkers are not uniformly available in the full public cohort; biomarker mechanisms should be treated as subset or external-anchor evidence. Third, knee-level analyses require participant-clustered uncertainty estimates in the final submission model. Fourth, NHANES anchors population composition and mortality but cannot validate OAI structural trajectory labels.

## Data availability

OAI public data are available through the Osteoarthritis Initiative data portal subject to its data-use terms. NHANES data are publicly available from the US Centers for Disease Control and Prevention. The derived analysis tables and source-data files generated by this script are intended for repository deposition after removal of any restricted or non-shareable fields according to OAI/NHANES terms.

## Code availability

The full Python analysis script used to generate the tables, figures and manuscript draft is included in the accompanying code folder. A frozen computational environment file should be added before submission.
"""
...
The OAI public ASCII files were read directly from `OAICompleteData_ASCII.zip`. The main mappings were: Duryea mJSW (`VxxMCMJSW`), KL grade (`VxxXRKL`), knee-specific WOMAC pain (`VxxWOMKPR/L`), WOMAC function (`VxxWOMADLR/L`), femorotibial angle (`VxxFTANGLE`), confirmed TKR (`V99ERKRPCF/V99ELKRPCF == 3`) and death (`V99EDDCF == 2`, days from baseline visit date to death date).

### Eligibility

Knees were eligible when at least two visits contained both mJSW and WOMAC pain. This criterion preserves longitudinal information while avoiding single-visit pseudo-trajectories.

### Trajectory model

The submitted Python implementation estimates knee-level intercepts and slopes for each marker and fits Gaussian mixture models to standardised trajectory features. Candidate class numbers 2-5 were compared using BIC. This is an auditable approximation to parallel-process latent class growth modelling and should be replaced or mirrored with `lcmm::multlcmm` or `hlme` for a final regulatory-style latent-trajectory appendix.

### Sensitivity analyses to run before final journal submission

1. Index-knee analysis using the more symptomatic baseline knee.
2. Definite radiographic OA subset (KL≥2).
3. Event definition including self-reported TKR (`V99*RPCF` in 1 or 3).
4. Cluster-robust uncertainty by participant ID.
5. Multiple imputation with at least 20 imputations.
6. MRI/MOAKS subset analysis with inverse-probability selection weights.
7. R Fine-Gray competing-risk analysis using death as the competing event.

## Supplementary Results

The eligible OAI set included {n_knees:,} knees from {n_participants:,} participants. Confirmed TKR occurred in {events:,} knees by 96 months; {deaths:,} deaths were identified in OUTCOMES99. The phenotype distribution and NHANES anchor results are provided in the source tables.
"""

    data_statement = """Data Availability
Osteoarthritis Initiative data used in this study are available from the OAI public data repository under the OAI data-use agreement. NHANES data used for population anchoring are publicly available from the Centers for Disease Control and Prevention/National Center for Health Statistics. Derived non-identifying aggregate tables and figure source data generated for this manuscript are provided with the analysis package and should be deposited in a public repository at submission, subject to the original data-use terms.

Repository and citation actions
- Add the final repository DOI for derived aggregate tables and figure source data before submission.
- Cite the OAI and NHANES source datasets according to their required acknowledgement language.

Missing information / risk flags
- Do not redistribute raw OAI participant-level files unless the OAI terms explicitly permit it.
- Add final software environment lock file and commit hash before submission.

中文核对
- 投稿前请确认 OAI 数据使用协议是否允许共享派生的膝级分析表；如果不允许，只共享汇总表、图源数据和代码。
"""

    cover_letter = f"""Dear Editor,

We are pleased to submit our manuscript, "Structure-symptom discordance phenotypes in knee osteoarthritis: joint structural-symptomatic trajectory subtyping, determinants, and total knee replacement risk in the Osteoarthritis Initiative, with population anchoring in NHANES," for consideration.

This study addresses a common but under-modelled problem in knee osteoarthritis: radiographic structure and patient symptoms often diverge. Rather than treating such divergence as a failed composite endpoint, we model discordance itself as a longitudinal phenotype. In {n_knees:,} eligible OAI knees, we identify structural-dominant and symptom-dominant phenotypes, evaluate their determinants, and quantify their relationship with total knee replacement risk. NHANES analyses further anchor high-symptom arthritis subgroups in a representative US population.

The work is potentially important for clinical triage and imaging stewardship. It suggests that high symptom burden with limited structural progression should prompt broader assessment of systemic, psychosocial and socioeconomic contributors, whereas high structural burden with limited symptoms may require monitoring without implying immediate surgical demand.

All analyses were conducted using reproducible code, and aggregate source data for figures and tables are prepared for repository deposition in accordance with OAI and NHANES data-use terms.
...
"""
    ...
    (MANUSCRIPT / "main_manuscript_draft.md").write_text(manuscript, encoding="utf-8")
    (SUPPLEMENT / "supplementary_methods_results.md").write_text(supplement, encoding="utf-8")
    (MANUSCRIPT / "data_availability.md").write_text(data_statement, encoding="utf-8")
    (MANUSCRIPT / "cover_letter.md").write_text(cover_letter, encoding="utf-8")
...
def main():
    ...
    med = mediation_like(panel, baseline, feat, pheno)
    ...
    write_csv(make_data_dictionary(panel, baseline, outcomes), SUPPLEMENT / "data_dictionary.csv")
    (TABLES / "symptom_gap_decomposition.json").write_text(json.dumps(med, indent=2), encoding="utf-8")

    plot_figures(panel, pheno, risks, dca, nh_prof)
    kinfo = {"structure_k": struct.k, "symptom_k": sympt.k}
    generate_text_outputs(summary, risks, inc, nh_prof, nh_mort, med, kinfo, baseline["id"].nunique())
    ...
    manifest = {
        ...
        "notes": [
            "Trajectory model is a Python two-stage approximation; mirror with lcmm for final latent growth model if required.",
